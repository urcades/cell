...
use cell_ai_core::{AssistantContentBlock, Model, StopReason};
use cell_ai_providers::{ProviderRegistry, ProviderRegistryError};
use cell_config::{SettingsManager, SettingsManagerError};
use cell_models::{
    ModelRegistry, ScopedModel, default_model_for_provider, resolve_cli_model,
    resolve_known_cli_model,
};
use cell_oauth::AuthSource;
use cell_protocol::OutputMode;
use cell_session::{SessionManager, SessionManagerError};
...
pub use cell_session::SessionTreeNode;
...
fn render_text_mode_response(
    assistant_message: &cell_ai_core::AssistantMessage,
...
mod tests {
    ...
    impl ApiProvider for EchoProvider {
        ...
        fn stream(
            ...
            let assistant = if let Some(cell_ai_core::Message::ToolResult(tool_result)) =
                last_message
            {
                AssistantMessage {
                    content: vec![AssistantContentBlock::Text {
                        text: format!(
                            ...
                            match &tool_result.content[0] {
                                cell_ai_core::UserContentBlock::Text { text, .. } =>
                                    text.clone(),
                                _ => "binary".to_string(),
                            }
                        ),
