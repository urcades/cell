...
use pi_rust_resources::{ContextDocument, SkillDefinition, format_skills_for_prompt};
...
pub fn build_system_prompt(options: BuildSystemPromptOptions<'_>) -> String {
        ...
        .filter(|value| !value.trim().is_empty())
    {
        ...
        let guidelines = build_guidelines(options.selected_tools)
            ...
            .collect::<Vec<_>>();
        format!(
            "You are an expert coding assistant operating inside pi-rust, a coding agent harness. You help users by reading files, executing commands, editing code, and writing new files.\n\nAvailable tools:\n{}\n\nGuidelines:\n{}",
            if tools.is_empty() {
                "(none)".to_string()
            } else {
                tools.join("\n")
