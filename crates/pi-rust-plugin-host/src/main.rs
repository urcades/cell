...
use pi_rust_plugin_host::{PluginHost, PluginHostConfig, discover_plugins};
...
fn run(args: Vec<String>) -> Result<(), Box<dyn std::error::Error>> {
    match args.as_slice() {
        [] => {
            eprintln!("usage: pi-rust-plugin-host discover <roots...> | launch <descriptor>");
            Ok(())
        }
        ...
        [command, descriptor] if command == "launch" => {
            ...
            let discovered = pi_rust_plugin_host::DiscoveredPlugin::load(descriptor)?;
