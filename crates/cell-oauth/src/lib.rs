    ...
    T: Send + 'static,
{
    ...
    thread::Builder::new()
        .name("cell-oauth-http".to_string())
        .spawn(move || {
...
mod tests {
    ...
    fn resolves_literal_and_env_backed_config_values() {
        unsafe { env::set_var("CELL_TEST_KEY", "secret") };
        assert_eq!(
            resolve_config_value("CELL_TEST_KEY").as_deref(),
            Some("secret")
        );
        assert_eq!(
        ...
        unsafe { env::remove_var("CELL_TEST_KEY") };
    }
