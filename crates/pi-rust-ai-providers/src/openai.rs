...
use pi_rust_oauth::{OAuthCredentials, OAuthProvider, register_oauth_provider};
use reqwest::Client;
...
async fn run_openai_completions(
    ...
    if model.provider.0 == "openrouter" {
        ...
        insert_header(&mut headers, "X-Title", "pi-rust");
    }
...
fn build_openai_codex_headers(
    ...
    insert_header(
        &mut headers,
        "user-agent",
        &format!("pi-rust/{}", env!("CARGO_PKG_VERSION")),
    );
...
fn convert_responses_input(
    ...
    let supports_images = model
        .input
        .iter()
        .any(|input| matches!(input, pi_rust_ai_core::ModelInput::Image));
...
mod tests {
    ...
    fn context() -> Context {
        Context {
            ...
            messages: vec![
                ...
                pi_rust_ai_core::Message::Assistant(AssistantMessage {
    ...
    fn builds_openai_responses_request_shape() {
        let request = build_openai_responses_request(
            ...
            &StreamOptions {
                max_tokens: Some(42),
                reasoning: Some(pi_rust_ai_core::AiThinkingLevel::High),
