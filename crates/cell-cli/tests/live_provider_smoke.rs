...
const PROVIDER_CASES: [ProviderCase; 4] = [
    ProviderCase {
        ...
        model_env: "CELL_SMOKE_MODEL_OPENAI",
    },
    ProviderCase {
        provider: "openai-codex",
        default_model: "gpt-5.3-codex",
        model_env: "CELL_SMOKE_MODEL_OPENAI_CODEX",
    },
    ProviderCase {
        provider: "openrouter",
        default_model: "openai/gpt-5.1-codex",
        model_env: "CELL_SMOKE_MODEL_OPENROUTER",
    },
    ProviderCase {
        provider: "anthropic",
        default_model: "claude-haiku-4-5",
        model_env: "CELL_SMOKE_MODEL_ANTHROPIC",
    },
];
...
fn run_cell(agent_dir: &Path, args: &[&str]) -> std::process::Output {
    let mut command = cargo_bin_cmd!("cell");
    command.env("CELL_CODING_AGENT_DIR", agent_dir);
    command.args(args);
    command.output().expect("run cell")
}
...
fn candidate_source_agent_dirs() -> Vec<PathBuf> {
    ...
    if let Ok(value) = env::var("CELL_SMOKE_SOURCE_AGENT_DIR") {
    ...
    if let Some(home) = env::var_os("HOME").map(PathBuf::from) {
        for candidate in [home.join(".cell/agent"), home.join(".pi/agent")] {
