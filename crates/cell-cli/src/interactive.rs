...
fn share_export_path() -> PathBuf {
    let millis = SystemTime::now()
        ...
        .unwrap_or(0);
    std::env::temp_dir().join(format!("cell-share-{millis}.html"))
}
...
mod tests {
    ...
    fn with_inline_images() -> TerminalCapabilities {
        TerminalCapabilities {
            kitty_keyboard: false,
            inline_images: true,
            image_protocol: Some(cell_tui::ImageProtocol::Kitty),
            hyperlinks: true,
        }
    }
    ...
    fn active_tool_render_lines_show_bash_output_text() {
        ...
        let rendered = lines
            .into_iter()
            .map(|line| match line {
                cell_tui::RenderedLine::Text(text) => strip_ansi(&text),
                cell_tui::RenderedLine::Image(_) => "[image]".to_string(),
            })
            .collect::<Vec<_>>();
    ...
    fn transcript_tool_result_uses_correlated_read_args_for_title_and_range() {
        let lines = session_transcript_lines(
            &[
                ...
                TranscriptEntry::Message(Message::ToolResult(cell_ai_core::ToolResultMessage {
    ...
    fn transcript_tool_result_formats_grep_results_with_structured_prefixes() {
        let lines = session_transcript_lines(
            &[TranscriptEntry::Message(Message::ToolResult(
                cell_ai_core::ToolResultMessage {
