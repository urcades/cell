...
use cell_ai_providers::{ProviderRegistry, register_builtin_providers};
use cell_config::{SettingsManager, SettingsScope, expand_tilde, get_agent_dir, get_project_config_dir};
use cell_core::{
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            CliError::UnsupportedExtensions => {
                write!(
                    f,
                    "cell v1 does not execute JS/TS extensions. Remove --extension/--no-extensions and any extension-only package resources."
                )
            }
            CliError::PackageCommandNotImplemented(command) => {
                write!(
                    f,
                    "cell recognizes the \"{}\" package command, but package execution is not implemented yet.",
                    command.as_str()
                )
            }
...
fn plugin_runtime_warning_from_host(
    warning: cell_plugin_host::PluginHostWarning,
) -> RpcPluginRuntimeWarning {
    RpcPluginRuntimeWarning {
...
mod tests {
    ...
    use cell_ai_providers::{ApiProvider, ProviderRegistry, register_builtin_providers};
    use cell_config::{ENV_AGENT_DIR, SettingsScope};
    use cell_models::ModelRegistry;
    use cell_oauth::AuthStorage;
    use cell_packages::{PackageInstallScope, PackageManager};
    use cell_plugins::{CommandRegistrationV1, PluginIdentityV1, PluginManifestV1};
...
"#
    ...
    fn package_validation_matches_typescript_shape() {
        ...
        assert_eq!(
            ...
            RunResult::Completed {
                ...
                stdout: None,
                stderr: Some(
                    "Unknown option --bogus for \"install\".\nUse \"cell --help\" or \"cell install <source> [-l]\"."
                        .to_string()
                ),
            }
        );
        ...
        assert_eq!(
            ...
            RunResult::Completed {
                ...
                stdout: None,
                stderr: Some(
                    "Missing remove source.\nUsage: cell remove <source> [-l]".to_string()
                ),
            }
        );
    }
  ...
  "openai-codex": {
    ...
    fn plugins_list_emits_json_diagnostics_for_discovered_plugins() {
        ...
        fs::write(
            plugin_root.join("cell-plugin-host.json"),
        ...
        let mut settings = cell_config::SettingsManager::create(&cwd, Some(agent_dir.clone()));
        settings
            .set_plugin_roots(
                cell_config::SettingsScope::Project,
                &["../plugins".to_string()],
            )
