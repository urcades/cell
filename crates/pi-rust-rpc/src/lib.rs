...
use pi_rust_core::{AgentControl, AgentSession, rpc_event_from_agent_event};
use pi_rust_jsonline_transport::{
    InboundFrame, JsonLineService, ServiceControl, TransportEmitter, run_stdio_service,
};
use pi_rust_protocol::{RpcCommand, RpcEvent, RpcInbound, RpcResponse};
use serde::Serialize;
use serde_json::json;
...
mod tests {
    ...
    fn assistant_message(model: &pi_rust_ai_core::Model, prompt: String) -> AssistantMessage {
        AssistantMessage {
